...
strictfp class SoldierStrategy {
    ...
    static void run(RobotController rc) throws GameActionException {  
        ...
        int numFriends = 0;
        for (RobotInfo info: rc.senseNearbyRobots(9, rc.getTeam())) {
            numFriends++;
            center = new MapLocation(center.x + info.getLocation().x, center.y + info.getLocation().y);
        }
        TripleTarget localTargets;
        if (numFriends != 0) {
            center = new MapLocation(center.x/numFriends, center.y/numFriends);
            localTargets = RobotPlayer.acquireLocalTargets(rc, target, enemies, me);
        }else {
            localTargets = RobotPlayer.acquireLocalTargets(rc, target, enemies, me);
        }
